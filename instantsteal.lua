...
local function instantClone()
    ...
    local ok, err = pcall(function()
        ...
        if not Workspace:FindFirstChild(cloneName) then error("Clone not found") end
        local vim2 = cloneref and cloneref(game:GetService("VirtualInputManager")) or VirtualInputManager
        local inset2 = (cloneref and cloneref(game:GetService("GuiService")) or GuiService):GetGuiInset()
        -- TeleportToClone button
        ...
        if tpButton then
            ...
            if firesignal then firesignal(tpButton.MouseButton1Up)
                ...
                local pos = tpButton.AbsolutePosition + (tpButton.AbsoluteSize / 2) + inset2
                vim2:SendMouseButtonEvent(pos.X, pos.Y, 0, true, game, 1)
                task.wait(0.05)
                vim2:SendMouseButtonEvent(pos.X, pos.Y, 0, false, game, 1)
            end
            return
        end
        -- Cherche "Change With Clone" dans tous les GUIs
        local foundBtn = nil
        for _, gui in ipairs(PlayerGui:GetDescendants()) do
            if gui:IsA("TextButton") or gui:IsA("ImageButton") then
                local txt = gui.Text or ""
                if txt:lower():find("change") and txt:lower():find("clone") then
                    foundBtn = gui; break
                end
                local lbl = gui:FindFirstChildWhichIsA("TextLabel", true)
                if lbl then
                    local lt = lbl.Text or ""
                    if lt:lower():find("change") and lt:lower():find("clone") then
                        foundBtn = gui; break
                    end
                end
            end
        ...
        if foundBtn then
            if firesignal then
                pcall(function() firesignal(foundBtn.MouseButton1Click) end)
                pcall(function() firesignal(foundBtn.MouseButton1Up) end)
            else
                local pos = foundBtn.AbsolutePosition + (foundBtn.AbsoluteSize / 2) + inset2
                vim2:SendMouseButtonEvent(pos.X, pos.Y, 0, true, game, 1)
                task.wait(0.05)
                vim2:SendMouseButtonEvent(pos.X, pos.Y, 0, false, game, 1)
            end
            ...
            local slotX = sz.X * 0.5 - 75 + 4 * 50
            vim2:SendMouseButtonEvent(slotX, sz.Y * 0.955, 0, true, game, 1)
            task.wait(0.05)
            vim2:SendMouseButtonEvent(slotX, sz.Y * 0.955, 0, false, game, 1)
        end
    end)
    _G.isCloning = false
end
...
task.spawn(function()
    ...
    local function createDuelBaseMarker(plot, sign)
        ...
        timerLbl.Name = "DuelTimer"
        timerLbl.Size = UDim2.new(1, 0, 0, 20)
        timerLbl.Position = UDim2.new(0, 0, 0, 30)
        timerLbl.BackgroundTransparency = 1
        timerLbl.Font = Enum.Font.GothamBold
        timerLbl.TextSize = 14
        timerLbl.TextColor3 = Color3.fromRGB(255, 200, 0)
        timerLbl.TextStrokeTransparency = 0
        timerLbl.TextStrokeColor3 = Color3.fromRGB(0, 0, 0)
        timerLbl.Text = "35s"
        duelBaseBillboards[plotName] = bb
        duelBaseStartTimes[plotName] = tick()
        task.spawn(function()
            while bb and bb.Parent do
                task.wait(0.5)
                local remaining = math.ceil(35 - (tick() - (duelBaseStartTimes[plotName] or tick())))
                if not (timerLbl and timerLbl.Parent) then break end
                if remaining > 0 then
                    timerLbl.Text = remaining .. "s"
                    timerLbl.TextColor3 = remaining > 10 and Color3.fromRGB(255,200,0) or Color3.fromRGB(255,60,60)
                else
                    if duelBaseHighlights[plotName] and duelBaseHighlights[plotName].Parent then duelBaseHighlights[plotName]:Destroy() end
                    duelBaseHighlights[plotName] = nil
                    if bb and bb.Parent then bb:Destroy() end
                    duelBaseBillboards[plotName] = nil
                    duelBaseStartTimes[plotName] = nil
                    break
                end
            end
        end)
...
local function runAutoSnipe()
    ...
    -- ÉTAPE 3 : Cloner à la position de la base (devant)
    -- On est déjà devant la base grâce aux TP précédents, on clone maintenant
    walkForward(0.25)
    task.wait(0.25)
    instantClone()
    local _cw = 0
    while _G.isCloning and _cw < 5 do task.wait(0.05); _cw = _cw + 0.05 end
    task.wait(0.1)

    -- ÉTAPE 4 : TP au brainrot (position du target)
    local verticalDiff2 = targetPart.Position.Y - hrp.Position.Y
    if verticalDiff2 > 2 then
        local airPos2 = Vector3.new(targetPart.Position.X, targetPart.Position.Y - 8, targetPart.Position.Z)
        for _i = 1, 10 do
            if LocalPlayer:GetAttribute("Stealing") then break end
            hrp.AssemblyLinearVelocity = Vector3.new(hrp.AssemblyLinearVelocity.X, 0, hrp.AssemblyLinearVelocity.Z)
            if (hrp.Position - airPos2).Magnitude > 3 then hrp.CFrame = CFrame.new(airPos2) end
            task.wait(0.05)
        end
    else
        for _i = 1, 10 do
            if LocalPlayer:GetAttribute("Stealing") then break end
            hrp.AssemblyLinearVelocity = Vector3.new(hrp.AssemblyLinearVelocity.X, 0, hrp.AssemblyLinearVelocity.Z)
            if (hrp.Position - targetPart.Position).Magnitude > 3 then hrp.CFrame = CFrame.new(targetPart.Position) end
            task.wait(0.05)
        end
    end
...
-- Boule bleue "Server Position" persistante
local _serverPosBall = nil
local _serverPosBallConn = nil

local function _createServerPosBall(position)
    if _serverPosBall and _serverPosBall.Parent then
        _serverPosBall.CFrame = CFrame.new(position)
        return
    end
    local ball = Instance.new("Part")
    ball.Name = "AquaServerPosBall"
    ball.Shape = Enum.PartType.Ball
    ball.Size = Vector3.new(2.5, 2.5, 2.5)
    ball.Color = Color3.fromRGB(0, 120, 255)
    ...
    lbl.BackgroundColor3 = Color3.fromRGB(0, 30, 80)
...
local function _destroyServerPosBall()
    if _serverPosBallConn then _serverPosBallConn:Disconnect(); _serverPosBallConn = nil end
    if _serverPosBall and _serverPosBall.Parent then _serverPosBall:Destroy() end
    _serverPosBall = nil
end
    ...
    if isStealing then
        ...
        if Config.DesyncOnSteal then
            task.spawn(function()
                ...
                for i = 1, 8 do
                    hrp.AssemblyLinearVelocity = Vector3.new(
                        math.random(-6, 6), hrp.AssemblyLinearVelocity.Y, math.random(-6, 6))
                    task.wait(0.015)
                end
                hrp.AssemblyLinearVelocity = Vector3.new(0, hrp.AssemblyLinearVelocity.Y, 0)
                -- Créer boule bleue à la position serveur et la suivre
                _createServerPosBall(hrp.Position)
                if _serverPosBallConn then _serverPosBallConn:Disconnect() end
                _serverPosBallConn = RunService.Heartbeat:Connect(function()
                    if not (LocalPlayer:GetAttribute("Stealing")) then
                        -- Steal terminé: garder la boule à la dernière position 2s puis destroy
                        if _serverPosBallConn then _serverPosBallConn:Disconnect(); _serverPosBallConn = nil end
                        task.delay(2, _destroyServerPosBall)
                        return
                    end
                    if _serverPosBall and _serverPosBall.Parent and hrp and hrp.Parent then
                        _serverPosBall.CFrame = CFrame.new(hrp.Position)
                    end
                end)
            end)
        end
...
task.spawn(function()
    ...
    local function createBrainrotBillboard(data)
        ...
        bb.LightInfluence = 0
        bb.MaxDistance = 3000
        -- Enregistre la première fois qu'on voit ce brainrot
        local seenAt = _espSeenTimes[data.uid]
        if not seenAt then
            seenAt = tick()
            _espSeenTimes[data.uid] = seenAt
        end
        ...
        nameLabel.Size = UDim2.new(1, -6, 0, 16); nameLabel.Position = UDim2.new(0, 3, 0, 2)
        nameLabel.BackgroundTransparency = 1; nameLabel.Font = Enum.Font.GothamBlack; nameLabel.TextSize = 12
        nameLabel.TextColor3 = color; nameLabel.TextStrokeTransparency = 0
        nameLabel.TextStrokeColor3 = Color3.fromRGB(0,0,0)
        nameLabel.Text = (data.name or data.petName) or "???"
        nameLabel.TextXAlignment = Enum.TextXAlignment.Center
        local genLabel = Instance.new("TextLabel", container)
        genLabel.Size = UDim2.new(1, -6, 0, 13); genLabel.Position = UDim2.new(0, 3, 0, 19)
        genLabel.BackgroundTransparency = 1; genLabel.Font = Enum.Font.GothamBold; genLabel.TextSize = 11
        genLabel.TextColor3 = Color3.fromRGB(255,255,255); genLabel.TextStrokeTransparency = 0
        genLabel.TextStrokeColor3 = Color3.fromRGB(0,0,0)
        genLabel.Text = data.genText or ""; genLabel.TextXAlignment = Enum.TextXAlignment.Center
        -- Timer ESP : temps écoulé depuis que le brainrot est visible
        local timerLabel = Instance.new("TextLabel", container)
        timerLabel.Name = "ESPTimer"
        timerLabel.Size = UDim2.new(1, -6, 0, 13); timerLabel.Position = UDim2.new(0, 3, 0, 34)
        timerLabel.BackgroundTransparency = 1; timerLabel.Font = Enum.Font.GothamBold; timerLabel.TextSize = 10
        timerLabel.TextColor3 = Color3.fromRGB(100,255,100); timerLabel.TextStrokeTransparency = 0.5
        timerLabel.TextStrokeColor3 = Color3.fromRGB(0,0,0); timerLabel.TextXAlignment = Enum.TextXAlignment.Center
        timerLabel.Text = "0s"
        task.spawn(function()
            while timerLabel and timerLabel.Parent do
                local elapsed = math.floor(tick() - seenAt)
                local m = math.floor(elapsed/60)
                timerLabel.Text = m > 0 and string.format("%dm%02ds", m, elapsed%60) or (elapsed.."s")
                timerLabel.TextColor3 = elapsed < 30 and Color3.fromRGB(100,255,100) or elapsed < 120 and Color3.fromRGB(255,200,0) or Color3.fromRGB(255,100,100)
                task.wait(1)
            end
        end)
        if hasMut then
            local mutBadge = Instance.new("TextLabel", bb)
            mutBadge.Size = UDim2.new(0,60,0,14); mutBadge.Position = UDim2.new(0.5,-30,0,-16)
            mutBadge.BackgroundColor3 = color; mutBadge.BackgroundTransparency = 0.3
            mutBadge.Font = Enum.Font.GothamBlack; mutBadge.TextSize = 9
            mutBadge.TextColor3 = Color3.fromRGB(255,255,255); mutBadge.TextStrokeTransparency = 0
            mutBadge.TextStrokeColor3 = Color3.fromRGB(0,0,0); mutBadge.Text = data.mutation:upper()
