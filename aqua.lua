...
task.spawn(function()
    ...
    local function createDuelBaseMarker(plot, sign)
        ...
        _duelStartTimes[plotName] = tick()
        task.spawn(function()
            while bb and bb.Parent do
                task.wait(0.5)
                local rem = math.ceil(35 - (tick() - (_duelStartTimes[plotName] or tick())))
                if not (timerLbl and timerLbl.Parent) then break end
                if rem > 0 then
                    timerLbl.Text = rem .. "s"
                    timerLbl.TextColor3 = rem > 10 and Color3.fromRGB(255,200,0) or Color3.fromRGB(255,60,60)
                else
...
local function runAutoSnipe()
    ...
    -- Clone APRÈS le TP (crée le clone à la position du target)
    ...
    if isStealing then
        ...
        if Config.DesyncOnSteal then
            task.spawn(function()
                ...
                local hrp = char and char:FindFirstChild("HumanoidRootPart")
                if not hrp then return end
                -- Boule bleue = position serveur visible
                local oldBall = Workspace:FindFirstChild("AquaServerPosBall")
                ...
                lbl.BackgroundColor3 = Color3.fromRGB(0, 20, 60)
                ...
                hrp.AssemblyLinearVelocity = Vector3.new(0, hrp.AssemblyLinearVelocity.Y, 0)
                -- Suit la position serveur pendant le steal
                local conn
...
task.spawn(function()
    ...
    local function createBrainrotBillboard(data)
        ...
        bb.LightInfluence = 0
        bb.MaxDistance = 3000
        local seenAt = _espSeen[data.uid] or tick()
        _espSeen[data.uid] = seenAt
        ...
        task.spawn(function()
            while timerLabel and timerLabel.Parent do
                local e = math.floor(tick() - seenAt)
                local m = math.floor(e/60)
                timerLabel.Text = m > 0 and string.format("%dm%02ds",m,e%60) or (e.."s")
                timerLabel.TextColor3 = e < 30 and Color3.fromRGB(100,255,100) or e < 120 and Color3.fromRGB(255,200,0) or Color3.fromRGB(255,100,100)
                task.wait(1)
            end
        end)
        if hasMut then
